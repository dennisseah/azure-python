...
def test_get_client(api_key, mocker: MockerFixture) -> None:
    mock_client = mocker.patch(
        "azure_python.services.azure_open_ai_chat_client_service.AzureOpenAIChatClient"
    )
    mock_def_cred = mocker.patch(
        "azure_python.services.azure_open_ai_chat_client_service.DefaultAzureCredential"
    )
